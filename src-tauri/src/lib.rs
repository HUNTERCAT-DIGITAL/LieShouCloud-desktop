...
fn remove_caption_win(title: &str) {
    unsafe {
        use windows::core::w;
        use windows::Win32::Foundation::{BOOL, HWND, LPARAM};
        use windows::Win32::UI::WindowsAndMessaging::*;
        let target: Vec<u16> = title.encode_utf16().collect();
        unsafe extern "system" fn cb(hwnd: HWND, lparam: LPARAM) -> BOOL {
            let data = &*(lparam.0 as *const (Vec<u16>, bool));
            let mut buf = [0u16; 256];
            let n = GetWindowTextW(hwnd, &mut buf);
            if n > 0 && buf[..n as usize] == data.0[..] {
        ...
        let mut data = (target, false);
        EnumWindows(Some(cb), LPARAM(&mut data as *mut (Vec<u16>, bool) as isize));
    }
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .on_page_load(|_window, _payload| {
            // 沉浸式：页面加载后窗口已创建——移除系统标题栏（setup 时机不可靠）
            #[cfg(target_os = "windows")]
            remove_caption_win("电网监控");
        })
