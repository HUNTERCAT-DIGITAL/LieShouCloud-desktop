...
describe("desktop finance service", () => {
  ...
  it("listLedger 带 type → query", async () => {
    ...
    expect(mockRequest).toHaveBeenCalledWith({ method: "GET", path: "/api/ledger?type=EXPENSE" });
  });
  ...
  it("deleteLedger → DELETE /ledger/{id}", async () => {
    ...
    expect(mockRequest).toHaveBeenCalledWith({ method: "DELETE", path: "/api/ledger/3" });
  });
