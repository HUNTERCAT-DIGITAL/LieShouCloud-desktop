...
export async function listTenants(): Promise<Tenant[]> {
  return request<Tenant[]>({ method: "GET", path: "/tenants" });
}

/** POST /tenants — 开通租户 */
export async function createTenant(body: CreateTenantRequest): Promise<Tenant> {
  return request<Tenant>({ method: "POST", path: "/tenants", body });
}

/** PUT /tenants/{id} — 更新租户 */
export async function updateTenant(id: number, body: UpdateTenantRequest): Promise<Tenant> {
  return request<Tenant>({ method: "PUT", path: `/tenants/${id}`, body });
}

/** DELETE /tenants/{id} — 删除租户 */
export async function deleteTenant(id: number): Promise<void> {
  return request<void>({ method: "DELETE", path: `/tenants/${id}` });
}
