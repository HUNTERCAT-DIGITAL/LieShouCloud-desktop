...
describe("desktop case service", () => {
  it("listCases → GET /api/legal/cases（baseUrl 为纯域名，path 必须带 /api）", async () => {
    mockRequest.mockResolvedValue({ items: [], total: 0, page: 0, size: 20 });
    await listCases();
    const call = mockRequest.mock.calls[0][0];
    expect(call.method).toBe("GET");
    expect(call.path).toBe("/api/legal/cases");
  });
  ...
  it("getCase → GET /legal/cases/{id}", async () => {
    ...
    await getCase(7);
    expect(mockRequest).toHaveBeenCalledWith({ method: "GET", path: "/api/legal/cases/7" });
  });
  ...
  it("deleteCase → DELETE /legal/cases/{id}", async () => {
    ...
    await deleteCase(3);
    expect(mockRequest).toHaveBeenCalledWith({ method: "DELETE", path: "/api/legal/cases/3" });
  });

  it("listCaseEvents → GET /legal/cases/{id}/events", async () => {
    mockRequest.mockResolvedValue([]);
    await listCaseEvents(5);
    expect(mockRequest).toHaveBeenCalledWith({ method: "GET", path: "/api/legal/cases/5/events" });
  });
  ...
  describe("计时确认流(后端 TimeEntryController)", () => {
    it("confirmTimeEntry → PUT /legal/time-entries/{id}/confirm", async () => {
      ...
      expect(mockRequest).toHaveBeenCalledWith({
        method: "PUT",
        path: "/api/legal/time-entries/7/confirm",
      });
    });
    ...
    it("deleteTimeEntry → DELETE /legal/time-entries/{id}", async () => {
      ...
      expect(mockRequest).toHaveBeenCalledWith({ method: "DELETE", path: "/api/legal/time-entries/9" });
    });
  });
  ...
  describe("费用编辑/删除(后端 ExpenseController)", () => {
    ...
    it("deleteExpense → DELETE /legal/expenses/{id}", async () => {
      ...
      expect(mockRequest).toHaveBeenCalledWith({ method: "DELETE", path: "/api/legal/expenses/5" });
    });
  });
  ...
  describe("文书登记/编辑/删除(后端 LegalDocumentController)", () => {
    it("createDocument → POST /legal/cases/{id}/documents + body", async () => {
      ...
      expect(mockRequest).toHaveBeenCalledWith({
        method: "POST",
        path: "/api/legal/cases/1/documents",
    ...
    it("deleteDocument → DELETE /legal/documents/{id}", async () => {
      ...
      expect(mockRequest).toHaveBeenCalledWith({ method: "DELETE", path: "/api/legal/documents/8" });
    });
  });
  ...
  describe("事件编辑/删除(后端新增 PUT/DELETE events)", () => {
    it("updateCaseEvent → PUT /legal/cases/{caseId}/events/{eventId} + body", async () => {
      ...
      expect(mockRequest).toHaveBeenCalledWith({
        method: "PUT",
        path: "/api/legal/cases/1/events/4",
    ...
    it("deleteCaseEvent → DELETE /legal/cases/{caseId}/events/{eventId}", async () => {
      ...
      expect(mockRequest).toHaveBeenCalledWith({ method: "DELETE", path: "/api/legal/cases/1/events/6" });
    });
  });
});
